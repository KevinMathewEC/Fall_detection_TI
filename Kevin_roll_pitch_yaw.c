...
int Ascale = AFS_4G;
int Mscale = MFS_16BITS; // Choose either 14-bit or 16-bit magnetometer resolution
int Mmode = 0x02;        // 2 for 8 Hz, 6 for 100 Hz continuous magnetometer data read
float aRes, gRes, mRes;      // scale resolutions per LSB for the sensors
float rms;//Stores the rms value of acceleration
...
//  Now = micros();//Returns the number of microseconds since the MSP432 board began running the current program. Data type: unsigned long.
 ...
 MadgwickAHRSupdate(gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, ax, ay, az,  my,  mx, mz);
//  MahonyQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, my, mx, mz);


    if (!AHRS) {
   // delt_t = millis() - count;//millis()-Number of milliseconds passed since the program started. Data type: unsigned long.
    //if(delt_t > 500) {
...
//    Serial.print(" qx = "); Serial.print(q1);
//    Serial.print(" qy = "); Serial.print(q2);
//    Serial.print(" qz = "); Serial.println(q3);
    }
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    pitch *= 180.0f / PI;
    yaw   *= 180.0f / PI;
    yaw   += 1.2667; /* Declination at RVCE, Bangalore ,India  Model Used:  WMM-2020
                                                               Latitude:  12° 54' 57" N
                                                               Longitude: 77° 30' 6" E
                                                               Date Declination
                                                               2020-02-28   1° 16' W  ± 0° 18'  changing by  0° 1' E per year*/
    roll  *= 180.0f / PI;
...
{
  int rawData[7];  // x/y/z gyro register data, ST2 register stored here, must read ST2 at end of data acquisition
...
// the accelerometer biases calculated above must be divided by 8.
  ...
  int mask_bit[3] = {0, 0, 0}; // Define array to hold mask bit for each accelerometer bias axis
...
void MPU9250SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
  ...
  writeByte(MPU9250_ADDRESS, CONFIG, 0x02);        // Set gyro sample rate to 1 kHz and Digital Low pass filter to 92 kHz
  writeByte(MPU9250_ADDRESS, GYRO_CONFIG, 1<<FS);  // Set full scale range for the gyro to 250 dps
  writeByte(MPU9250_ADDRESS, ACCEL_CONFIG2, 0x02); // Set accelerometer rate to 1 kHz and bandwidth to 92 kHz
...
// Configure the accelerometer for self-test
  ...
  for( int ii = 0; ii < 100; ii++) {  // get average self-test values of gyro and acclerometer
  ...
  for (int ii =0; ii < 3; ii++) {  // Get average of 100 values and store as average self-test readings
  aSTAvg[ii] /= 100;
  gSTAvg[ii] /= 100;
  }
